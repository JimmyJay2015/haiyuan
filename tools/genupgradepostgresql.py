#coding=utf-8
"""
本脚本用来从hsqldb的 openfire.script 生成插入到postgresql的数据。
主要处理包括：
1，过滤非insert数据
...
if __name__ == "__main__":
    if len(sys.argv)!=6:
        print "input parameters: srcpath destpath database olddomain newdomain"
        exit()
        
    srcpath = sys.argv[1]
    destpath = sys.argv[2]
    database = sys.argv[3]
    olddomain = sys.argv[4]
    newdomain = sys.argv[5]
    ...
    while True:
        ...
        if line.find('INSERT') !=0:
            ...
            #domain 替换
            insert_sql = insert_sql.replace(olddomain,newdomain)
